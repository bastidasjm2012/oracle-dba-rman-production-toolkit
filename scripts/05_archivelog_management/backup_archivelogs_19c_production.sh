#!/bin/bash
# =============================================================================
# Script Name : backup_archivelogs_19c_production.sh
# Description : Enterprise wrapper for Oracle RMAN archivelog backup
...
BACKUP_DIR="${BACKUP_BASE}/${ORACLE_SID}/archivelog/${DATE_TAG}"
LOG_DIR="${BACKUP_BASE}/${ORACLE_SID}/logs"
LOG_FILE="${LOG_DIR}/archivelog_backup_${ORACLE_SID}_${DATE_TAG}.log"
RMAN_SCRIPT="$(dirname "$0")/backup_archivelogs_19c_production.rman"

mkdir -p "$BACKUP_DIR" "$LOG_DIR"

echo "============================================================" | tee -a "$LOG_FILE"
echo "Oracle RMAN Archivelog Backup - Production Wrapper" | tee -a "$LOG_FILE"
...
if [ "$RMAN_STATUS" -ne 0 ]; then
    echo "ERROR: RMAN archivelog backup failed. Check log: $LOG_FILE"
    exit "$RMAN_STATUS"
fi

echo "SUCCESS: RMAN archivelog backup completed successfully."
