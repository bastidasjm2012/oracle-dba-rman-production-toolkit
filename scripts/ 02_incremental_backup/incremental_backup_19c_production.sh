#!/bin/bash
# =============================================================================
# Script Name : incremental_backup_19c_production.sh
# Description : Enterprise wrapper for Oracle RMAN incremental database backup
...
BACKUP_DIR="${BACKUP_BASE}/${ORACLE_SID}/incremental/${DATE_TAG}"
LOG_DIR="${BACKUP_BASE}/${ORACLE_SID}/logs"
LOG_FILE="${LOG_DIR}/incremental_backup_${ORACLE_SID}_${DATE_TAG}.log"
RMAN_SCRIPT="$(dirname "$0")/incremental_backup_19c_production.rman"

mkdir -p "$BACKUP_DIR" "$LOG_DIR"

echo "============================================================" | tee -a "$LOG_FILE"
echo "Oracle RMAN Incremental Backup - Production Wrapper" | tee -a "$LOG_FILE"
...
if [ "$RMAN_STATUS" -ne 0 ]; then
    echo "ERROR: RMAN incremental backup failed. Check log: $LOG_FILE"
    exit "$RMAN_STATUS"
fi

echo "SUCCESS: RMAN incremental backup completed successfully."
